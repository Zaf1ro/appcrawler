...
XPATH_APP_SIZE = "//div[@class='pf']/span[4]"
...
class QihooSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for name, category_url in CATEGORY:
            yield Request(
                url=category_url,
                callback=self.parse,
    ...
    def parse_app(response):
        app_item = QihooItem()
        app_item['category'] = response.meta['']
        ...
        app_item['download_count'] = response.xpath(XPATH_DOWNLOAD_TIMES)
        app_item['apk_size'] = response.xpath(XPATH_APP_SIZE)
        yield app_item
